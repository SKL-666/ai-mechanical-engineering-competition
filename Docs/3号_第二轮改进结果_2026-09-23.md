...
验证集选择预处理为`S2_unweighted`，最终训练模式为`stream`。部署候选由seed42验证连续Macro-F1选择为`temporal_stream_s42`，没有按测试分数反选。
...
| S1_intervals | 39.05% |
| S2_unweighted | 46.47% |
| S3_letterbox96 | 38.82% |
| S4_letterbox128 | 37.27% |
| S5_augmentation | 33.27% |
| S6_union | 34.47% |

当前20epoch条件下，S2优于S1，支持先取消这组类别权重；等比补边/放大/增强/联合框未优于被选方案。这些是有限验证集与训练预算下的结果，不代表相关技术普遍无效。
...
|---|---:|---:|---:|
| single | 34.51 ± 3.83 | 20.28 ± 5.06 | 21.18 ± 5.61 |
| temporal | 40.59 ± 9.24 | 24.84 ± 6.03 | 25.79 ± 8.72 |

![三种子对比](../experiments/03_action_recognition/20260923_round2/comparison.png)
...
|---|---:|---:|---:|---:|
| single/42 | 17.06% | 17.90% | 9.71% | 49.47% |
| temporal/42 | 24.97% | 25.09% | 32.98% | 52.57% |
| single/3407 | 17.66% | 17.98% | 7.39% | 50.14% |
| temporal/3407 | 30.80% | 34.84% | 31.22% | 62.49% |
| single/2026 | 26.11% | 27.65% | 28.10% | 49.15% |
| temporal/2026 | 18.75% | 17.44% | 9.52% | 47.97% |

## 每类与逐视频

| 模型 | 类别 | 每次测试support | F1均值±标准差 |
|---|---|---:|---:|
| single | idle | 10 | 7.00 ± 1.56 |
| single | swing_bucket | 80 | 56.19 ± 7.00 |
| single | load_bucket | 44 | 16.02 ± 10.11 |
| single | dump | 35 | 22.17 ± 19.68 |
| single | move | 1 | 0.00 ± 0.00 |
| temporal | idle | 10 | 12.13 ± 1.23 |
| temporal | swing_bucket | 80 | 60.99 ± 3.49 |
| temporal | load_bucket | 44 | 28.66 ± 28.49 |
| temporal | dump | 35 | 22.44 ± 23.46 |
| temporal | move | 1 | 0.00 ± 0.00 |

| 模型 | 视频 | 片段Accuracy均值 | 片段Macro-F1均值 |
|---|---|---:|---:|
| single | 104154 | 29.82% | 15.54% |
| single | 134516 | 38.30% | 22.35% |
| temporal | 104154 | 25.00% | 15.43% |
| temporal | 134516 | 53.19% | 28.76% |

Move的测试support始终只有1；3个随机种子反复评价的是同一例，不能当成3条独立测试动作。仍未删除类别、补标签或扩大场景数量。
...
`test_continuous_raw_<video>/probabilities.jsonl`由独立的禁止真值访问进程生成；`selected_output/action_segments.csv`在读取评价真值之前产生。帧/秒均左闭右开，按视频与设备ID组织。所有连续结果是XML真值框原型，未使用2号真实检测/跟踪，未执行4号工序判断。
...
原件核验数量：89108。Data、原始ZIP和指导文件按SHA256及清单验证不变；v1与round2索引哈希不变，全部连续末帧目标和历史框检查通过。六份最终checkpoint不同，测试发生在freeze事件之后，全部测试support与原170片段一致。
