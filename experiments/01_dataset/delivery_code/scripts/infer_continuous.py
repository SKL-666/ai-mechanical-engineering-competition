...
def reject_label_access(event, args):
    if event == 'open' and isinstance(args[0], (str, bytes)):
        path = args[0].decode() if isinstance(args[0], bytes) else args[0]
        path = path.replace('\\', '/')
        if ('/Labels/' in path or path.endswith(('excavator.txt', 'clips.jsonl', 'windows.jsonl',
                                                 'raw_intervals.jsonl', '_consensus.npy'))):
            raise RuntimeError(f'Action truth access forbidden in continuous inference: {path}')


def hold_end(boxes, video, equipment, start, end):
    return next((t for t in range(start, end) if (video, equipment, t) not in boxes), end)
...
def infer(a):
    ...
    # Fail at runtime if this entry point (including imported helpers) tries to read action truth.
    sys.addaudithook(reject_label_access)
    ...
    for eid in ids:
        ...
        for end in range(window, n + 1, stride):
            ...
            if any((a.video, eid, t) not in boxes for t in range(start, end)):
                continue
            # Stop holding a prediction as soon as the CURRENT track disappears.
            # This is track availability handling, never action-boundary-triggered slicing.
            stop = hold_end(boxes, a.video, eid, emit, stop)
    ...
    write_json(a.out / 'inference_config.json', {
        ...
        'action_truth_access_guard': 'Python audit hook raises on action label/index/consensus file access',
